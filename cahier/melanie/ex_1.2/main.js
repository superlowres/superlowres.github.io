...
function draw(){
	...
	const x1 = Math.sin(frameCount*0.01)*0.4
	...
	for (let j=0; j<NUM_Y; j++){
		for (let i=0; i<NUM_X; i++){
			...
			const t = (j * 8 - NUM_Y) / NUM_Y
			const v = (j * 6 - NUM_Y) / NUM_Y
			const w = (j * 4 - NUM_Y) / NUM_Y
			const s = (j * 2 - NUM_Y) / NUM_Y

			// on calcule la distance de chaque celle par raport a un "centre"
			let d = 1e100
			// cercle 1
			//d = Math.min(dist(0, 0, u+x1, v+y1) - 0.1, d)
			d = Math.min(dist(0, 0, w+x1, v*0.2+y1) - 0.1, d)
			d = Math.min(dist(0, 0, s+x1, v*0.4+y1) - 0.1, d)
			d = Math.min(dist(0, 0, t+x1, v*0.8+y1) - 0.1, d)
			d = Math.min(dist(0, 0, u+x1, v*'0.2'+y1) - 0.1, d)
			// cercle 2
	...
	for(let j=0; j<NUM_Y; j++) {
		for(let i=0; i<NUM_X; i++) {
			...
			const idx = i + j * NUM_X
			const v = data[idx]
			fill(v * random(255), random(100),100)
