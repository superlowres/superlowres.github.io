...
//tableau3
const x7 = Math.sin(frameCount*0.011) * 0.05;
const y7 = Math.cos(frameCount*0.052) *0.04;
const x8 = Math.sin(frameCount*0.013) * 0.04;
const y8 = Math.cos(frameCount*0.014) *0.05;
const x9 = Math.sin(frameCount*0.5) * 0.04;
const y9 = Math.cos(frameCount*0.12) * 0.09;
...
for (let j=0; j<NUM_Y; j++){
	for (let i=0; i<NUM_X; i++){
		...
		let d = 1e100
		// cercle 1
		d = Math.min(dist(0, 0, u+x7, v+y7) -mid, d)

		//cercle 2
		d = Math.min(dist(0, 0, u-x8, v-y8) -mid, d)

		//cercle 3
		d = Math.min(dist(0, 0, u-x9, v-y9) -mid, d)  // ICI tu creer un cercle rm = largeur mid & high = largeur high & rb = largeur bass x
