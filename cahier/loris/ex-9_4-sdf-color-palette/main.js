	...
	for (let j = 0; j < NUM_Y; j++) {
		for (let i = 0; i < NUM_X; i++) {
			...
			if (v1 < 1 && v1 > 0) {
				...
				let pal_idx = Math.floor(map(v1, 0, 1, currPalette[0].length - 1, 0))
			...
			// if (v2 < 1 && v2 > -1) {
			// 	pal_idx = Math.floor(map(v2, 0, 1, currPalette[0].length - 1, 0))
			// 	fill(currPalette[val][pal_idx])
			// 	if (v2 < 1) {
			...
			// 	pal_idx = Math.floor(map(v3, 0, 1, 0, currPalette[0].length - 1))
