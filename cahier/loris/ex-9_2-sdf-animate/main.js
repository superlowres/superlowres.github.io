...
function draw() {
	...
	for (let j = 0; j < NUM_Y; j++) {
		for (let i = 0; i < NUM_X; i++) {
			...
			let val = radio.value()
			if (val == 1) {
				tx = Math.sin(-frameCount * .01) * .5
				ty = -Math.abs(Math.sin(frameCount * .01) * .5)
				x = u + tx
				y = v + ty
				d1 = sdBox(x, y, w, h, a)
				s = 'move on x/y'
			...
			if (val == 2) {
				...
				d1 = sdBox(xRot, yRot, w, h, a)
				s = 'rotate on origin'
			}
			if (val == 3) {
				tx = Math.sin(frameCount * .01) * .5
				ty = Math.cos(frameCount * .01) * .5
				x = u + tx
				y = v + ty
				xRot = x * cos(-a) - y * sin(-a)
				yRot = x * sin(-a) + y * cos(-a)
			...
			if (val == 4) {
				h = .4 +Math.sin(-frameCount * .2) * .2
				w = .4 +Math.cos(frameCount * .2) * .2
				d1 = sdBox(x, y, w, h, a)
			   s = 'animate size'
			}
			if (val == 5) {
				h = .4 +Math.sin(-frameCount * .1) * .2
				w = .4 +Math.cos(frameCount * .1) * .2
				tx = Math.sin(frameCount * .06) * .5
				ty = Math.cos(frameCount * .01) * .5
				x = u + tx
				y = v + ty
				xRot = x * cos(-a) - y * sin(-a)
